...
def load_dataset_from_json(json_path, images_base_path="data/images"):
    """Load dataset from JSON file and prepare for SFT training"""
    ...
    for item in data:
        # Get image path
        image_path = os.path.join(images_base_path, item.get('image_path', ''))
        ...
        # Format: User provides image and question, Assistant provides reasoning + answer
        user_message = item.get('question', '')
        
        # Construct the expected response with reasoning and solution format
        reasoning = item.get('heur_reason', '')
...
train_conversations = load_dataset_from_json(TRAIN_JSON)
...
if os.path.exists(VAL_JSON):
    ...
    val_conversations = load_dataset_from_json(VAL_JSON)
